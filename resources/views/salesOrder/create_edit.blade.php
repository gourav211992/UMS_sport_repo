...
@section('content')
    ...
    <!-- BEGIN: Content-->
    <form method="POST" data-completionFunction = "disableHeader" class="ajax-input-form sales_module_form" action = "{{route('sale.order.store')}}" data-redirect="{{ request() -> type == 'sq' ? route('sale.quotation.index') : route('sale.order.index') }}" id = "sale_order_form" enctype='multipart/form-data'>
                ...
                @include('layouts.partials.breadcrumb-add-edit', [
					...
					<div class="content-header-right text-sm-end col-md-6 mb-50 mb-sm-0">
                        ...
                        <button type = "button" onclick="javascript: history.go(-1)" class="btn btn-secondary btn-sm mb-50 mb-sm-0"><i data-feather="arrow-left-circle"></i> Back</button>  
                            @if (isset($order))
                                <a href="{{route('sale.order.generate-pdf', $order->id)}}" target="_blank" class="btn btn-dark btn-sm mb-50 mb-sm-0 waves-effect waves-float waves-light">
                                                                    ...
                                                                    @foreach ($order -> items as $orderItemIndex => $orderItem)
                                                                        ...
                                                                        <input type = 'hidden' name = "so_item_id[]" value = "{{$orderItem -> id}}" {{$orderItem -> is_editable ? '' : 'readonly'}}>
                                                                         <td class="customernewsection-form">
                                                                            <div class="form-check form-check-primary custom-checkbox">
                                                                                <input {{$orderItem -> restrict_delete ? 'disabled' : ''}} type="checkbox" class="form-check-input item_row_checks" id="item_checkbox_{{$orderItemIndex}}" del-index = "{{$orderItemIndex}}">
...
<div class="modal fade" id="approveModal" tabindex="-1" aria-labelledby="shareProjectTitle" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <form class="ajax-submit-2" method="POST" action="{{ route('document.approval.so') }}" data-redirect="{{ request() -> type == 'sq' ? route('sale.quotation.index') : route('sale.order.index') }}" enctype='multipart/form-data'>
          @csrf
...
<script>
        ...
        function addOrderDiscountInTable(index)
        {
                ...
                for (let index = newIndex - 1; index < previousHiddenNameFields.length; index++) {
                    // console.log(previousHiddenPercentageFields[index].value, previousHiddenValuesFields[index].value, "ORDER DISCOUNT");
...
document.addEventListener('input', function (e) {
    ...
    const checkBomCondition = debounce(async function (itemIndex) {
                $.ajax({
                ...
                data: {
                    ...
                    item_attributes : document.getElementById('items_dropdown_'+ itemIndex).getAttribute('attribute-array'),
                },
                success: function(data) {
