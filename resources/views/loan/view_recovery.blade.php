                                            ...
                                            <div class="col-md-3">
                                                <div class="row align-items-center mb-1">
                                                    ...
                                                    <div class="col-md-12">
                                                        ...
                                                        <input type="text"
                                                            value="{{ \App\Helpers\Helper::formatIndianNumber($data->rec_interest_amnt) }}"
    ...
    <script>
        ...
        function get_total2() {
            ...
            document.querySelectorAll('#recovery_history_2 tr').forEach(row => {
                ...
                totalDisbursed += parseFloat(row.querySelector('#rec_dis_amnt2_' + row.rowIndex)
                    ?.innerText || 0);
                totalRecovered += parseFloat(row.querySelector('#rec_rec_amnt2_' + row.rowIndex)
                    ?.innerText || 0);
                totalBalance += parseFloat(row.querySelector('#rec_blnc_amnt2_' + row.rowIndex)?.innerText ||
                    0);
                totalInterest += parseFloat(row.querySelector('#rec_intrest_amnt2_' + row.rowIndex)
                    ?.innerText || 0);
                totalSettledInterest += parseFloat(row.querySelector('#rec_set_intrst2_' + row.rowIndex)
                    ?.innerText ||
                    0);
                totalSettledPrincipal += parseFloat(row.querySelector('#rec_set_princ2_' + row.rowIndex)
                    ?.innerText ||
                    0);
                totalRemaining += parseFloat(row.querySelector('#rec_remaining2_' + row.rowIndex)
                    ?.innerText || 0);
            });
