...
@section('content')
        ...
        <div class="content-wrapper container-xxl p-0">
            ...
            <div class="content-body">
                ...
                <form action="{{ route('loan.recovery.add-update') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <input type="hidden" name="settled_interest" id="settled_interest" value="">

                    <input type="hidden" name="settled_principal" id="settled_principal" value="">
    ...
    if (input.id !== 'document_no') {
                ...
                if (customerID != "") {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            var totalDisbursement = data.customer_record.loan_disbursements
                                .reduce(
                                    function(sum, disbursement) {
                                        if (disbursement.document_status === "Disbursed") {
                            ...
                            $("#loan_amount").val((data.customer_record
                                .loan_appraisal
                                .term_loan));
                            $("#rec_intrst").val((totalIntrestReceived));
                            $("#rec_intrst_in").val(totalIntrestReceived);
                            $("#dis_amount").val((totalDisbursement));
                            $("#rec_amnt").val((
                            ...
                            $("#balance_amount").val((
                                balance_amount.toFixed(2))); // Set balance amount
                            ...
                            $.each(data.customer_record.loan_disbursements, function(ky, dis) {

                                if(dis.document_status=='Disbursed'){
                                ...
                                let dis_remaining = dis.remaining ? (
                                    dis.remaining) : 0
                                $('#recovery_history_2').append(
                                    `<tr>
                                <td> ${key + 1}</td>
                                <td id="rec_date2_${key+1}">${ created_date|| ""}</td>
                                <td  id="rec_dis_amnt2_${key+1}" class="fw-bolder text-dark text-end">${((parseFloat(removeCommas(dis.actual_dis) ?? removeCommas(dis.dis_amount) )) || 0)}</td>
                                <td  id="rec_rec_amnt2_${key+1}" class="text-end">${(parseFloat(dis.recovered) || 0)}</td>
                                <td  id="rec_blnc_amnt2_${key+1}" class="text-end">${(parseFloat(value))}</td>
                        ...
                        if (recovery_remaining > 0) {
                            ...
                            if (interest > 0) {
                                if (recovery_remaining >= interest) {
                                    ...
                                    settledInterestCell.textContent = (interest);
                                    settledInterestCell2.text((interest));
                                } else {
                    ...
                    if (recovery_remaining > 0) {
                        ...
                        if (principal > 0) {
                            if (recovery_remaining >= principal) {
                                ...
                                settledPrincipalCell.textContent = (principal);
                                settledPrincipalCell2.text((principal));


                            } else {
                                settledPrincipalCell.textContent = (recovery_remaining);
        ...
        var formattedAmount = (removeCommas(amount));
        ...
        function get_total2() {
            ...
            document.getElementById('rec_dis_amnt2').innerText = (totalDisbursed);
            document.getElementById('rec_rec_amnt2').innerText = (totalRecovered);
            document.getElementById('rec_blnc_amnt2').innerText = (totalBalance);
            document.getElementById('rec_intrest_amnt2').innerText = (totalInterest);
            document.getElementById('rec_set_intrst2').innerText = (totalSettledInterest);
            document.getElementById('rec_set_princ2').innerText = (totalSettledPrincipal);
            document.getElementById('rec_remaining2').innerText = (totalRemaining);
        ...
        function adjust_remaining() {
            ...
            document.getElementById('settled_interest').value = totalSettledInterest;
            document.getElementById('settled_principal').value =totalSettledPrincipal;


           // console.log(totalSettledInterest);
            //document.getElementById(totalSettledPrincipal);
