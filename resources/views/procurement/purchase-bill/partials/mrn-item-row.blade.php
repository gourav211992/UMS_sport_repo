@foreach($mrnItems as $key => $item)
    ...
    <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
        ...
        <td>
            <input type="hidden" class="form-control mw-100 mrn_qty" name="components[{{$rowCount}}][mrn_qty]" value="{{($item->accepted_qty - $item->purchase_bill_qty)}}" />
            <input type="number" class="form-control mw-100 accepted_qty text-end checkNegativeVal" name="components[{{$rowCount}}][accepted_qty]" value="{{($item->accepted_qty - $item->purchase_bill_qty)}}" step="any"/>
        </td>
        <td>
