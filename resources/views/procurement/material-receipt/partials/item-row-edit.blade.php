@foreach($mrn->items as $key => $item)
   ...
   <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
      ...
      <td>
         <input type="text" placeholder="Select" class="form-control mw-100 ledgerselecct" name="store_section" value="{{$item->store_code}}" />
         <input type="hidden" name="components[{{$rowCount}}][store_id]" value="{{$item->store_id}}">
         <input type="hidden" name="components[{{$rowCount}}][erp_store_code]" value="{{$item->store_code}}">
      </td>
      <td>
         <div class="d-flex">
