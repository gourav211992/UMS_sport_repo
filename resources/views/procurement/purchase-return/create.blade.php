...
@section('content')
    <form class="ajax-input-form" method="POST" action="{{ route('purchase-return.store') }}" data-redirect="/purchase-return" enctype="multipart/form-data">
        ...
        <input type="hidden" name="pr_qty_type" id="pr_qty_type" value="">
        @csrf
    ...
    <script>
            ...
            fetch(actionUrl).then(response => {
                return response.json().then(data => {
                    if (data.status == 200) {
                        ...
                        if(parameters?.pr_qty_type.some(val => val.toLowerCase() === 'rejected')) {
                            $("#pr_qty_type").val(parameters?.pr_qty_type[0]);
                        } else {
                            $("#pr_qty_type").val(parameters?.pr_qty_type[1]);
                        }
                        setTableCalculation();
                    }
                    if(data.status == 404) {
        ...
        function initializeAutocomplete1(selector, type) {
            $(selector).autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    let document_date = $("[name='document_date']").val();
                    let actionUrl = "{{route('purchase-return.get.address')}}"+'?id='+itemId+'&document_date='+document_date;
                    fetch(actionUrl).then(response => {
                        return response.json().then(data => {
        ...
        function getItemDetail(currentTr) {
            ...
            if (itemId) {
                ...
                let actionUrl = '{{route("purchase-return.get.itemdetail")}}'+'?item_id='+itemId+'&mrn_header_id='+mrnHeaderId+'&mrn_detail_id='+mrnDetailId+'&selectedAttr='+JSON.stringify(selectedAttr)+'&remark='+remark+'&uom_id='+uomId+'&qty='+qty;
                fetch(actionUrl).then(response => {
                    return response.json().then(data => {
                        if(data.status == 200) {
        ...
        function getMaterialReceipts() 
        {
            ...
            let pr_qty_type = $("#pr_qty_type").val() || '';
            let type = '{{ request()->route("type") }}';
            let actionUrl = '{{ route("purchase-return.get.mrn", ["type" => ":type"]) }}'.replace(':type', type);
            let fullUrl = `${actionUrl}?series_id=${encodeURIComponent(series_id)}&document_number=${encodeURIComponent(document_number)}&item_id=${encodeURIComponent(item_id)}&vendor_id=${encodeURIComponent(vendor_id)}&header_book_id=${encodeURIComponent(header_book_id)}&pr_qty_type=${encodeURIComponent(pr_qty_type)}`;
        ...
        $(document).on('click', '.poProcess', (e) => {
            ...
            .replace(':type', type) + '?ids=' + ids + '&pr_qty_type=' + pr_qty_type;
            fetch(actionUrl).then(response => {
                return response.json().then(data => {
                    if(data.status == 200) {
