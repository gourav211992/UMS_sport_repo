@foreach($mrnItems as $key => $item)
    ...
    <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
        ...
        @if($qtyTypeRequired && ($qtyTypeRequired == 'rejected'))
            <td>
                <input type="hidden" class="form-control mw-100 mrn_qty" name="components[{{$rowCount}}][mrn_qty]" value="{{($item->rejected_qty - $item->pr_qty)}}" />
                <input type="number" class="form-control mw-100 accepted_qty text-end checkNegativeVal" name="components[{{$rowCount}}][accepted_qty]" value="{{($item->rejected_qty - $item->pr_qty)}}" step="any"/>
            </td>
        @else 
            <td>
                <input type="hidden" class="form-control mw-100 mrn_qty" name="components[{{$rowCount}}][mrn_qty]" value="{{($item->accepted_qty - $item->pr_qty)}}" />
                <input type="number" class="form-control mw-100 accepted_qty text-end checkNegativeVal" name="components[{{$rowCount}}][accepted_qty]" value="{{($item->accepted_qty - $item->pr_qty)}}" step="any"/>
            </td>
        @endif
        <td>
