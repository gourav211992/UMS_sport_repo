                                        ...
                                        <div class="row">
                                            <div class="col-md-8">
                                                <div class="row align-items-center mb-1">
                                                    ...
                                                    <div class="col-md-5">
                                                        <input type="hidden" name="book_id" value="{{$mrn->book_id}}">
                                                        <select class="form-select" disabled id="book_id" name="book_id" readonly>
                                                        @foreach($books as $book)
                                                            <option value="{{$book->id}}" {{$book->id == $mrn->book_id ? 'selected' : ''}}>{{$book->book_code}}</option>
                                                        @endforeach
                                                        </select>
                                                        <input type="hidden" name="book_code" id="{{$mrn->book->book_code}}" id="book_code">
                                                    </div>
                                                </div>
                                                <div class="row align-items-center mb-1">
                                                    <div class="col-md-3"> 
        ...
        function initializeAutocomplete1(selector, type) {
            $(selector).autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    let actionUrl = "{{route('purchase-return.get.address')}}"+'?id='+itemId;
                    fetch(actionUrl).then(response => {
                        return response.json().then(data => {
                            if(data.status == 200) {
