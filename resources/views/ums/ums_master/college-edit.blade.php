				...
				<div class="row">
					...
					<div class="content-header-right text-sm-end col-md-6 mb-50 mb-sm-0">
						<div class="form-group breadcrumb-right">   
							...
							<button  type="submit" form="submitForm" data-bs-target="#disclaimer" class="btn btn-primary btn-sm mb-50 mb-sm-0"><i data-feather="check-circle"></i> Update</button> 
						</div>
					</div>
				</div>
			</div>
                                                        ...
                                                        <select class="form-select @error('institute_id') is-invalid @enderror" name="institute_id">
                                                            ...
                                                            @foreach ($instituteData as $institute)
                                                                <option value="{{ $institute->id }}" 
                                                                    {{ old('institute_id', $collegeMapping->institute_id) == $institute->id ? 'selected' : '' }}>
                                                        ...
                                                        <select class="form-select @error('program_type_id') is-invalid @enderror" name="program_type_id" id="programTypeDropdown">
                                                            ...
                                                            @foreach ($programTypeData as $type)
                                                                <option value="{{ $type->id }}" 
                                                                    {{ old('program_type_id', $collegeMapping->program_type_id) == $type->id ? 'selected' : '' }}>
                                                                    {{ $type->program_name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        ...
                                                        <select class="form-select @error('course_id') is-invalid @enderror" name="course_id" id="courseDropdown">
                                                            ...
                                                            @foreach ($courseData as $course)
                                                                <option value="{{ $course->id }}" 
                                                                    {{ old('course_id', $collegeMapping->course_id) == $course->id ? 'selected' : '' }}>
                                                                    {{ $course->course_name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        ...
                                                        <div class="demo-inline-spacing">
                                                            <div class="form-check form-check-primary mt-25">
                                                                <input type="radio" id="active" name="status" class="form-check-input" value="1" 
                                                                    {{ old('status', $collegeMapping->status) == '1' ? 'checked' : '' }}>
                                                            ...
                                                            <div class="form-check form-check-primary mt-25">
                                                                <input type="radio" id="inactive" name="status" class="form-check-input" value="0" 
                                                                    {{ old('status', $collegeMapping->status) == '0' ? 'checked' : '' }}>
                                            ...
                                            @foreach ($branches as $index => $branchItem)
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <input type="hidden" name="program_branch_id[]" value="{{ $branchItem['program_branch_id'] }}">
                                                    </td>
                
                                                    {{-- Semester Type --}}
                                                    <td>
                                                        <select class="form-select mw-100" name="semester_type[]">
                                                            <option value="1" {{ old('semester_type.' . $index, $branchItem['semester_type']) == '1' ? 'selected' : '' }}>1</option>
                                                            <option value="2" {{ old('semester_type.' . $index, $branchItem['semester_type']) == '2' ? 'selected' : '' }}>2</option>
                                                            <option value="3" {{ old('semester_type.' . $index, $branchItem['semester_type']) == '3' ? 'selected' : '' }}>3</option>
                                                        </select>
                                                    </td>
                
                                                    {{-- Course Duration --}}
                                                    <td>
                                                        <input type="text" class="form-control mw-100" name="course_duration[]" value="{{ old('course_duration.' . $index, $branchItem['course_duration']) }}">
                                                    </td>
                
                                                    {{-- Max Course Duration --}}
                                                    <td>
                                                        <input type="text" class="form-control mw-100" name="max_course_duration[]" value="{{ old('max_course_duration.' . $index, $branchItem['max_course_duration']) }}">
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
