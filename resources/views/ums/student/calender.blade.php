            ...
            <div class="content-header row">
                <div class="content-header-left col-md-5 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-start mb-0">Student Holiday Calender</h2>
                                    ...
                                    <table class="datatables-basic table myrequesttablecbox tableistlastcolumnfixed newerptabledesignlisthome">
                                        <thead>
                                            <tr>
                                                <th>SN#</th>
                                                <th>Year</th>
                                                <th>Click on image to View Calendar</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @if($holidayCalendor->count() > 0)
                                                @foreach($holidayCalendor as $index => $row)
                                                <tr>
                                                    <td>{{++$index}}</td>
                                                    <td>{{$row->year}}</td>
                                                     <td><a href="{{$row->HolidayCalenderModel_doc}}" target="_blank"><img src="{{$row->HolidayCalenderModel_doc}}" style="width:75px;"></a></td>
                                                    <!-- <td>@if($row->HolidayCalenderModel_doc)<a href="{{$row->HolidayCalenderModel_doc}}" target="_blank">Download</a>@endif</td> -->
                                                    
                                                </tr>
                                                @endforeach
                                            @else
                                                <tr>
                                                    <th colspan="3">Record not found</th>
                                                </tr>
                                            @endif
                                        </tbody>
                                    </table>
