...
@section('content')
                                            ...
                                            <div class="contract-details">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-3 mb-75 col-6">
                                                        ...
                                                        <p class="font-small-3">
                                                            @php
                                                                $subActivities = json_decode($data['sub_activities'], true);
                                                            @endphp
                                                    ...
                                                    <table class="table myrequesttablecbox table-striped po-order-detail custnewpo-detail border newdesignerptable newdesignpomrnpad">
                                                            ...
                                                            <tr>
                                                                ...
                                                                <th>
                                                                    <div class="form-check form-check-primary mt-25 custom-checkbox">
                                                                        ...
                                                                        <label class="form-check-label" for="checkAll">Present (Y/N)</label>
                                                                    </div>
                                                                </th>
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <input type="hidden" name="students[{{ $stu->id }}][attendance]" value="absent">
                                                                    ...
                                                                    <td>
                                                                        <select name="students[{{ $stu->id }}][absence_reason]" class="form-select">
                                                                            <option value="">Select</option>
                                                                            @foreach(['Tournament', 'Rest', 'Injury', 'Sickness', 'Weekly Holiday', 'Sanctioned Leave', 'Unauthorised Leave'] as $reason)
                                                                                <option value="{{ $reason }}" @if(($data['absence_reason'] ?? '') === $reason) selected @endif>{{ $reason }}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </td>
                                                        
                                                                    <td>
                                                                        <select name="students[{{ $stu->id }}][activity_comment]" class="form-select">
                                                                            <option value="">Select</option>
                                                                            @foreach(['Tournament', 'Rest', 'Injury', 'Sickness', 'Weekly Holiday', 'Sanctioned Leave', 'Unauthorised Leave'] as $comment)
                                                                                <option value="{{ $comment }}" @if(($data['activity_comment'] ?? '') === $comment) selected @endif>{{ $comment }}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </td>
                                                        
                                                                    <td>
                                                                        <select name="students[{{ $stu->id }}][rating]" class="form-select">
                                                                            <option value="">Select</option>
                                                                            @for ($i = 5; $i >= 1; $i--)
                                                                    ...
                                                                    <td>
                                                                        <input type="text" name="students[{{ $stu->id }}][color]" class="form-control"
                                                                               value="{{ $data['color'] ?? '' }}">
                                                                    </td>
                                                        
                                                                    <td>
                                                                        <input type="text" name="students[{{ $stu->id }}][remarks]" class="form-control"
                                                                               value="{{ $data['remarks'] ?? '' }}">
                                                                    </td>
                                                                </tr>
                                                            @endforeach
                                                    ...
                                                    <table class="table myrequesttablecbox table-striped po-order-detail custnewpo-detail border newdesignerptable newdesignpomrnpad">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th>
                                                                    <div class="form-check form-check-primary mt-25 custom-checkbox">
                                                                        <input type="checkbox" class="form-check-input mt-0" id="checkAll" disabled>
                                                                        <label class="form-check-label" for="checkAll">Present (Y/N)</label>
                                                                    </div>
                                                                </th>
                                                                <th>Absence Reason</th>
                                                        ...
                                                        <tbody>
                                                            @foreach ($students as $index => $stu)
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        {{-- Hidden input to mark default "absent" --}}
                                                                        <input type="hidden" name="students[{{ $stu->id }}][attendance]" value="absent">
                                        
                                                                        <input disabled type="checkbox" class="present-checkbox"
                                                                               name="students[{{ $stu->id }}][attendance]"
                                                                               value="present"
                                                                               data-id="{{ $stu->id }}"
                                                                               @if(!empty($data['attendance']) && $data['attendance'] === 'present') checked @endif>
                                                                    ...
                                                                    <td>
                                                                        {{-- Displaying selected absence reason as text --}}
                                                                        <span>{{ $data['absence_reason'] ?? 'Not Provided' }}</span>
                                                                    </td>
                                                                    
                                                                    <td>
                                                                        {{-- Displaying selected activity comment as text --}}
                                                                        <span>{{ $data['activity_comment'] ?? 'Not Provided' }}</span>
                                                                    </td>
                                                                    
                                                                    <td class="  name="students[{{ $stu->id }}][rating]"
                                                                            class="form-select attendance-related attendance-{{ $stu->id }} rating-select"
                                                                            data-id="{{ $stu->id }}"">
                                                                        {{-- Displaying selected rating as text --}}
                                                                        <span>{{ $data['rating'] ?? 'Not Rated' }}</span>
                                                                    </td>
                                                                    {{-- <td>
                                                                        <select
    ...
    <script>
        function toggleFields(studentId, isPresent) {
            const fields = document.querySelectorAll('.attendance-' + studentId);
        ...
        document.querySelectorAll('.present-checkbox').forEach(checkbox => {
            ...
            toggleFields(studentId, checkbox.checked); // Initial load
    
            checkbox.addEventListener('change', function () {
                toggleFields(studentId, this.checked); // On toggle
            });
        });
        ...
        document.getElementById('checkAll')?.addEventListener('change', function () {
            document.querySelectorAll('.present-checkbox').forEach(cb => {
                ...
                toggleFields(cb.dataset.id, cb.checked);
            });
        });
    </script>
