...
<body class="vertical-layout vertical-menu-modern navbar-floating footer-static menu-collapsed" data-open="click"
                                                ...
                                                <div class="col-md-12">
                                                    ...
                                                    <div class="demo-inline-spacing">
                                                        <div class="form-check form-check-primary mt-25">
                                                            <input type="radio" id="customColorRadio3" name="status" value="active" class="form-check-input" {{ $sport_screening->status == 'active' ? 'checked' : '' }}>
                                                            <label class="form-check-label fw-bolder" for="customColorRadio3">Active</label>
                                                        </div>
                                                        <div class="form-check form-check-primary mt-25">
                                                            <input type="radio" id="customColorRadio4" name="status" value="inactive" class="form-check-input" {{ $sport_screening->status == 'inactive' ? 'checked' : '' }}>
    ...
    <script>
        $(document).ready(function () {
            ...
            // Update serial numbers after rows are added
        ...
        function updateSerialsAndNames() {
            $('#parameter-table-body tr').each(function (index) {
                if ($(this).hasClass('add-template')) return;  // Skip the template row
                $(this).find('.sno').text(index + 1); // Update serial number to be sequential
            });
        }

        // Function to collect data for the form submission (in case you need it)
        function collectJsonData() {
            let parameterDetails = [];
            $('#parameter-table-body tr').each(function () {
                if ($(this).hasClass('add-template')) return; // Skip the template row
                let parameterName = $(this).find('.parameter-input').val();
                if (parameterName) {
                    parameterDetails.push({ parametername: parameterName });
                }
            });
