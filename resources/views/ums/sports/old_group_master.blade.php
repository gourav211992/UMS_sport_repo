                                                ...
                                                <td class="tableactionnew">
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            ...
                                                            <a class="dropdown-item" href="#" onclick="if(window.confirm('Are you sure you want to delete this data?')) { deleteGroup('{{$item->id}}'); }">
...
<div class="modal modal-slide-in fade filterpopuplabel" id="filter">
    <div class="modal-dialog sidebar-sm">
        <form class="add-new-record modal-content pt-0" id="approveds-form" method="GET" novalidate action="{{url('group-master')}}"> 
            @csrf
            <div class="modal-header mb-1">
                <h5 class="modal-title" id="exampleModalLabel">List of groups</h5>
            ...
            <div class="modal-body flex-grow-1">
                <div class="mb-1">
                    <div class="form-group">
                        ...
                        <input type="text" name="group_name" class="form-control" value="{{Request::get('group_name')}}">
                    </div>
                </div>
 ...
 {{-- content --}}
 {{-- <script>
    // AJAX request for deleting a group
    function deleteGroup(groupId) {
        // Confirm before deleting
        if (confirm('Are you sure you want to delete this data?')) {
            // Log the groupId and URL for debugging
            console.log("Group ID to delete:", groupId);
            console.log("URL:", '{{ route('group-master-delete', '') }}/' + groupId);
            
            // Perform AJAX request
            $.ajax({
                url: '{{ route('group-master-delete', '') }}/' + groupId,  // Append the group ID to the URL
                type: 'DELETE',
                data: {
                ...
                error: function(xhr, status, error) {
                    alert('Something went wrong. Please try again.');  // Handle error
                    console.error(xhr.responseText); // Log the error message in the console
                }
            });
        } else {
...
<script>
    ...
    function deleteGroup(slug) {
		var url = "{{url('group-master-delete')}}"+"/"+slug;
        // alert(url);
		window.location.href = url;
	}
</script>
