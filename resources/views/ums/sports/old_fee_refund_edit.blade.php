            ...
            <div class="row">
                        ...
                        <div class="col-12">
                            ...
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">Fee Refund  Edit</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                ...
                <div class="content-header-right text-sm-end col-md-6 mb-50 mb-sm-0">
                    <div class="form-group breadcrumb-right">   
                        ...
                        <button data-bs-toggle="modal" type="submit" form="submitData" data-bs-target="#disclaimer" class="btn btn-primary btn-sm mb-50 mb-sm-0"><i data-feather="check-circle"></i> Update</button> 
                    </div>
                </div>
            </div>
        </div>
        <div class="content-body">
            <form id="submitData" method="POST" action="{{ route('FeeRefund_Update', $feeRefund->id) }}">
                @csrf
...
<div class="row align-items-center mb-1">
    ...
    <div class="col-md-5">
        <select class="form-select" id="batchSelect" name="batch_id">
            ...
            <option value="all" {{ old('batch_id', $feeRefund->batch_id) == 'all' ? 'selected' : '' }}>All</option>
            @foreach($batches as $batch)
                <option value="{{ $batch->id }}" {{ old('batch_id', $feeRefund->batch_id) == $batch->id ? 'selected' : '' }}>
                    {{ $batch->batch_name }}
                </option>
            @endforeach
        </select>
...
<div class="row align-items-center mb-1">
    ...
    <div class="col-md-5">
        <select class="form-select" id="sectionSelect" name="section_id">
            <option {{ old('section_id', $feeRefund->section_id) == 'all' ? 'selected' : '' }}>---Select Section---</option>
    ...
    <div class="col-md-5">
        ...
        @enderror
                                            ...
                                            <div class="row align-items-center mb-1">
                                                ...
                                                <div class="col-md-5">
                                                    <input type="number"  step="0.01" class="form-control" id="refund_balance" name="refund_balance" value="{{ old('refund_balance', $feeRefund->total_refunded) }}">
                                            ...
                                            <div class="row align-items-center mb-1">
                                                ...
                                                <div class="col-md-5">
                                                    <select class="form-select" name="refund_method" id="refundMethodSelect" required>
                                                        ...
                                                        <option value="UPI" {{ old('refund_method', $feeRefund->refund_method) == 'UPI' ? 'selected' : '' }}>UPI</option>
                                                        <option value="Paytm" {{ old('refund_method', $feeRefund->refund_method) == 'Paytm' ? 'selected' : '' }}>Paytm</option>
                                                        <option value="Cheque" {{ old('refund_method', $feeRefund->refund_method) == 'Cheque' ? 'selected' : '' }}>Cheque</option>
                                                    </select>
                                                    @error('refund_method')
                                                    <small class="text-danger">{{ $message }}</small>
                                                    @enderror
                                            ...
                                            <div class="row align-items-center mb-1">
                                                ...
                                                <div class="col-md-5"> 
                                                    <input type="text" maxlength="27" class="form-control" id="transaction_number" name="transaction_number" value="{{ old('transaction_number',$feeRefund->transaction_number) }}">
                                            ...
                                            <div class="row align-items-center mb-1">
                                                ...
                                                <div class="col-md-5">
                                                    <input type="date" name="refund_date" class="form-control" value="{{ old('refund_date', $feeRefund->refund_date) }}">
                                                    @error('refund_date')
                                                    <small class="text-danger">{{ $message }}</small>
                                                @enderror
                                                </div>
                                            ...
                                            <div class="row align-items-center mb-1">
                                                ...
                                                <div class="col-md-5">
                                                    <input type="text" name="reason" class="form-control" value="{{ old('reason', $feeRefund->reason) }}">
                                                    @error('reason')
                                                    <small class="text-danger">{{ $message }}</small>
                                                @enderror
                                                </div>
                                            ...
                                            <div class="row align-items-center mb-1">
                                                ...
                                                <div class="col-md-5">
                                                    <input type="text"  name="approved_by" class="form-control" value="{{ old('approved_by', $feeRefund->approved_by) }}">
                                                    @error('approved_by')
                                                    <small class="text-danger">{{ $message }}</small>
                                                @enderror
                                                </div>
...
<script>
    $(document).ready(function () {
        ...
        const oldBatchId = "{{ old('batch_id', $feeRefund->batch_id) }}";
        const oldSectionId = "{{ old('section_id', $feeRefund->section_id) }}";
        const oldStudentId = "{{ old('registration_id', $feeRefund->registration_id) }}";
        ...
        // ✅ Allow typing freely without formatting issues
        $('#refund_balance').on('input', function () {
            calculateRefundBreakdown();
        });

        // ✅ Format to 2 decimal places only after focus leaves the field
        $('#refund_balance').on('blur', function () {
            let value = $(this).val();
    ...
    let feePaid = parseFloat($('#feePaidInput').val()) || 0;

    // Check if refundBalance is more than feePaid
    if (refundBalance > feePaid) {
        refundBalance = 0;
        $('#refund_balance').val(refundBalance.toFixed(2)); // Reset field to 0
    }

    let refundBreakdown = feePaid - refundBalance;
