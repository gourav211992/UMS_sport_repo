...
@section('scripts')
    ...
    <script>
        ...
        const uploadRoute =
            "{{ isset($parent) ? route('document-drive.file.upload', [$parent]) : route('document-drive.file.upload') }}";
        document.querySelector('#file-upload').addEventListener('change', function() {
            ...
            if (files.length > 0) {
                ...
                Array.from(files).forEach((file, index) => {
                    ...
                    const dynamicRoute = uploadRoute;
            ...
            `;
                    ...
                    xhr.onload = function() {
                        if (xhr.status === 200) {
                            ...
                            document.getElementById('uploading').textContent = "Uploaded";
                            document.getElementById('upload-count').textContent = response
        ...
        document.querySelector('#folder-upload').addEventListener('change', function() {
            ...
            if (files.length > 0) {
                ...
                Array.from(files).forEach((file, index) => {
                    ...
                    // Replace placeholder in route with actual parentId
                    const dynamicRoute = uploadRoute2;
        ...
        $(document).on('click', '#moveSubmit', function() {
            ...
            const destinationFolderId =  $('input[name="destination_folder_id"]:checked').val();;
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    // Check if the server responded with errors in a partial success case
    ...
    rows.forEach(row => {
    const nameCell = row.querySelector('td:nth-child(2)'); // Select the 'Name' column (2nd column)
    const nameText = nameCell ? nameCell.textContent.toLowerCase() : ''; // Get the name text in lowercase

    // Get the hidden input with class 'item_tags'
    const itemTagsInput = row.querySelector('.tags_filter');
    let tagsArray = [];

    if (itemTagsInput) {
        try {
            // Parse the JSON value inside the 'value' attribute of the input
            const jsonValue = JSON.parse(itemTagsInput.value);
            // Assuming the JSON structure contains an 'index' key holding the tags
            tagsArray = jsonValue.index || [];
        } catch (e) {
            console.error("Error parsing JSON from 'item_tags' input:", e);
        }
    }

    // Check if the name or any of the tags contain the search term
    const searchTermLower = searchTerm.toLowerCase();  // Make search term lowercase
    if (nameText.includes(searchTermLower) || tagsArray.some(tag => tag.toLowerCase().includes(searchTermLower))) {
        ...
        row.style.display = 'none'; // Hide row if it doesn't match
                ...
                folderItems.forEach(function(folder) {
                    // Get the folder name from <h6> and convert to lowercase for comparison
                    const folderName = folder.querySelector('h6').textContent.toLowerCase();

                    // Get the hidden data from the <h1> tag
                    const hiddenValue = folder.querySelector('h1').textContent.toLowerCase();
                    let hiddenTags = [];

                    if (hiddenValue!="") {
                        if (hiddenValue) {
                            // Parse the JSON content from the hidden <h1> tag's value if it exists
                            try {
                                const jsonValue = JSON.parse(hiddenValue);
                                hiddenTags = jsonValue.index || []; // Assume 'index' is the key holding tag values
                            } catch (e) {
                                console.error("Error parsing JSON in <h1>: ", e);
                            }
                        }
                    }

                    // Check if the search term matches either folder name or any tag in the hidden data
                    if (folderName.includes(searchTerm.toLowerCase()) || (hiddenTags.length > 0 && hiddenTags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())))) {
    ...
    // Get the file name from <h6> and convert to lowercase for comparison
    const fileName = file.querySelector('h6').textContent.toLowerCase();

    // Get the hidden data from the <h1> tag
        const hiddenValue = file.querySelector('h1').textContent.toLowerCase();
    let hiddenTags = [];
    ...
    // Check if the search term matches either file name or any tag in the hidden data
    if (fileName.includes(searchTerm.toLowerCase()) || (hiddenTags.length > 0 && hiddenTags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())))) {
        file.style.display = 'block';  // Show file
    } else {
                    ...
                    <div class="content-body">
                        ...
                        <div class="file-manager-main-content">
                                ...
                                <div class="d-flex align-items-center">
                                    <div class="input-group input-group-merge shadow-none m-0 flex-grow-1">
                                        ...
                                        <input type="text" class="form-control files-filter border-0 bg-transparent"
                                            placeholder="Search" id="search-folder-all" />
                                    </div>
                                </div>
                                <div class="d-flex align-items-center">
                                    <div class="file-actions d-block">
                                        ...
                                        <span data-bs-toggle="tooltip" data-bs-placement="bottom" class="delete-btn-all"
                                            ...
                                            data-redirect="{{ isset($parent) ? route('document-drive.folder.show', [$parent]) : route('document-drive.index') }}"
                            ...
                            <div class="file-manager-content-body grid-data">
                                <div class="">
                                    <h6 class="folders-section-title mt-25 mb-75">Folders</h6>
                                    <div class="row">
                                    ...
                                    <tbody>
                                        @foreach ($items as $item)
                                            <tr class="cursor-pointer">
                                                ...
                                                <input type="hidden" class="tags_filter"
                                                value="{{ $item->tags }}">
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-sm">
            <div class="modal-content p-0">
                <div class="modal-header">
                    <h5 class="modal-title"><span id="uploading">Uploading</span> <span id="upload-count">0</span> Items
                    </h5>
                                                ...
                                                <td class="pe-0">
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input" id="destinationFolder" type="radio"
                                                            name="destination_folder_id" value="root"
                                                            required>
                                                    </div>
                                                </td>
                                                <td class="fw-bolder text-dark">
    ...
    <div class="modal modal-slide-in fade filterpopuplabel" id="filter">
        <div class="modal-dialog sidebar-sm">
            ...
            <form class="add-new-record modal-content pt-0" method="GET"
                action="{{ isset($parent) ? route('document-drive.folder.show', [$parent]) : route('document-drive.index') }}">
