...
<td><input type="number" class="form-control mw-100 text-end" value="{{$po_item->order_qty - $po_item->invoice_quantity}}" name="components[{{$rowCount}}][qty]" @if($po_item->pi_item_id) maxAmount="{{$po_item?->pi_item?->balence_qty}}" @endif step="any"></td>
<td><input @readonly(true) type="number" name="components[{{$rowCount}}][rate]" value="{{$po_item->rate}}" class="form-control mw-100 text-end" /></td> 
<td><input type="number" value="{{$po_item->order_qty * $po_item->rate}}" name="components[{{$rowCount}}][item_value]" class="form-control mw-100 text-end" step="any" /></td>
<td>
