...
function updateRecoveryScheduleTable(data) {
    ...
    data.forEach((item, index) => {
        ...
        row.innerHTML = `
            <td>${index + 1}</td>
            <td>${formattedDate}</td>
            <td>${item.lease_cost || "N/A"}</td>
            <td>${item.received_amount || "N/A"}</td>
            <td>${item.bal_lease_cost || 0}</td>
            <td>${totalReceived}</td>
            <td>${daysOverdue}</td>
        `;
...
function createTableRow(report, index, visibleColumnIndices) {
    ...
    let paymentDate = report.recovery[0]?.date_of_payment ?? null;
    ...
    const cellsData = [
        ...
        report.customer ?? "N/A",
        report.land_no ?? "N/A",
        report.area_sqft ?? "N/A",
        report.cost ?? "N/A",
        report.lease_cost ?? "N/A",
        report.lease_time ?? "N/A",
        report.period_type ?? "N/A",
        report.installment_cost ?? "N/A",
        report.recovery_sum_received_amount ?? "N/A",
        report.recovery[0]?.bal_lease_cost ?? "N/A",
        daysDifference ?? "N/A",
        `<a href="${window.routes.landReportFilter}"><i class="cursor-pointer" data-feather='eye'></i></a>
        <a href="#" class="schedulerReportBtn" data-bs-toggle="modal" data-bs-target="#landReportModal" data-land-no="${report.land_no}"><i data-feather="filter"></i></i></a>`,
    ];
