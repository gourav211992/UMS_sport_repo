...
$(document).on('change',"[name*='order_qty']",(e) => {
    ...
    // if(orderQty < 1) {
    //     orderQuantity.val(oldValue);
    //     acceptedQuantity.val(oldValue);
    //     rejectedQuantity.val(oldValue);
    //     Swal.fire({
    //         title: 'Error!',
    //         text: 'Receipt Quantity must be positive integer.',
    //         icon: 'error',
    //     });
    //     return false;
    // } else{
...
$(document).on('change',"[name*='accepted_qty']",(e) => {
    ...
    if(Number(acceptedQuantity.val()) > Number(receiptQuantity.val())) {
        ...
        if(mrnDetailId || poDetailId){
            let actionUrl = '/material-receipts/validate-quantity?item_id='+itemId+'&mrnDetailId='+mrnDetailId+'&poDetailId='+poDetailId+'&qty='+acceptedQuantity.val();
