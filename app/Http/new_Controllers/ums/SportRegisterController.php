...
use App\Models\ums\SportStudentLogin as User;
//use App\Models\User;
...
//    dd($user);
    $student = \App\models\User::find($user->id);
//    dd($student);
...
//    dd($quotas);
    ...
    $feeDetails = json_decode($sportFeeMaster->fee_details, true);
        ...
        $user = \App\models\User::with('payments')->findOrFail($user->id);
...
//        return redirect()->back()->with('error', 'Registration failed. Please try again.')->withInput();
    ...
    //     $students = SportRegister::with('SportBatch')->latest()->get();
...
// {
//     $query = SportRegister::with('SportBatch')
...
public function confirm(  Request $request, $id){
    ...
    public function edit($id)
    {
        ...
        $user = \App\models\User::with('payments')->findOrFail($registration->userable_id);
...
//        dd($request->all());
        ...
        $registration = SportRegister::findOrFail($id);
        $user = User::find($registration->userable_id);
...
//        dd($student->registration);
        ...
        $user = \App\models\User::with('payments')->findOrFail($id);
...
//        $student = User::where('email',$user->email)->first();
        ...
        $otherStates = State::where('country_id', $registration->bai_state)->get();
        $user = User::with('payments')->find($registration->userable_id);
//        dd($feeDetails);
...
//        dd($student->registration);
    ...
    public function update_payment(Request $request)
    {
        try {
            ...
            $payment = SportPayment::updateOrCreate(
...
//            $user->save();
            ...
            return response()->json([
                'success' => true,
                'message' => 'SportPayment updated successfully!'
            ]);
    ...
    public function update_payment_status(Request $request)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'SportPayment status updated to paid!'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
