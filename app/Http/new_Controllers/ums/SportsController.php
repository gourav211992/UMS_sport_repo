...
use App\Models\ums\SportStudentLogin;
...
class SportsController extends Controller
{
    public function register(Request $request){
        ...
        $super = AuthUser::where('organization_alias',$prefix)->first();
        $authUser = new AuthUser;
        $authUser->name = $request->first_name . ' ' . $request->middle_name ?? '' . ' ' . $request->last_name ??'';
//        $authUser->user_name = explode('@', $request->email)[0];
        $authUser->mobile = $request->mobile;
        $authUser->email = $request->email;
        $authUser->password = Hash::make($request->password);
        $authUser->user_type = 'IAM-STUDENT';
        $authUser->authenticable_type = 'sport_student';
        $authUser->authenticable_id = $user->id;
        $authUser->db_name = $super->db_name;
        $authUser->group_id = $super->group_id;
        $authUser->company_id = $super->company_id;
        $authUser->organization_id = $super->organization_id;
        $authUser->organization_alias = $super->organization_alias;
        $authUser->organization_name = $super->organization_name;
        $authUser->save();
        if ($user) {
            $mobile = $user->mobile;
...
//            dd($response);
    ...
    public function verifyOTP(Request $request) {
        ...
        $user = SportStudentLogin::where('email', $request->email)->first();
    ...
    public function verifyEmail(Request $request) {
        ...
        $user = SportStudentLogin::where('email_verification_token', $token)->first();
        if ($user) {
