...
class ExpenseAdviseController extends Controller
{
    ...
    {   
        ...
        $html = view('procurement.expense-advise.partials.comp-item-detail',compact('item','purchaseOrder', 'saleOrder', 'selectedAttr','remark','uomName','qty','specifications'))->render();
        return response()->json(['data' => ['html' => $html], 'status' => 200, 'message' => 'fetched.']);
    }
    ...
    public function getSo(Request $request)
    {
        ...
        $poItems = ErpSoItem::select(
            ...
            ->get();
        $soItemIds = $poItems->pluck('id');

        $soItems = PoItem::with('attributes')->whereIn('id',$soItemIds)
