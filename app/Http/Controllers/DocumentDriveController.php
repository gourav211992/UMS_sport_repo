...
use App\Models\ErpDocumentDriveFolder;  // Reference for folder relationship
use App\Models\ErpDocumentDriveSharedResource;
use App\Models\ErpDocumentDriveActionLog;
...
class DocumentDriveController extends Controller
{
    public function index(Request $request)
    {
        ...
        $fileQuery = ErpDocumentDriveFile::where('created_by', $userData->id)->where('created_by_type',get_class($userData))->whereNull('folder_id');
        $folderQuery = ErpDocumentDriveFolder::where('created_by', $userData->id)->where('created_by_type',get_class($userData))->whereNull('parent_id');
        ...
        $all_folders = ErpDocumentDriveFolder::where('created_by', $userData->id)->where('created_by_type',get_class($userData))->get();
    ...
    public function show(string $id,Request $request)
    {
        ...
        $fileQuery = ErpDocumentDriveFile::where('folder_id', $id)
            ->where('created_by', $userData->id)->where('created_by_type',get_class($userData));
        ...
        $items = $folders->merge($files)->sortBy('name');
        $all_folders = ErpDocumentDriveFolder::where('created_by', $userData->id)->where('created_by_type',get_class($userData))
        ->where('id','!=',$id)->get();
        $users = User::where('id', '!=', $userData->id)->get();
    ...
    public function sharedWithMe(Request $request, $id = null)
    {
        ...
        $sharedResources = ErpDocumentDriveSharedResource::where('shared_with', $userData->id)->get();
    ...
    public function sharedDrive(Request $request,$id=null)
    {
        ...
        // Get shared resources for the current user
        $sharedResources = ErpDocumentDriveSharedResource::where('shared_by', $userData->id)->get();
...
public function upload(Request $request, $parentId = null)
{
    ...
    Log::info('User data fetched.', ['userData' => $userData, 'authUser' => $authUser]);
        ...
        try {
            if ($file->isValid()) {
                ...
                ErpDocumentDriveFile::create([
                    ...
                    'created_by_type' => get_class($userData),
                    'size' => $file->getSize(),
    ...
    public function create_folder(Request $request, $parentId = null)
    {
        ...
        if ($parentId) {
            ...
            $existingFolder = ErpDocumentDriveFolder::whereNull('parent_id')
                ...
                ->where('created_by_type',get_class($userData))
                ->first();
        }
        ...
        $folder->created_by = $userData->id;
        $folder->created_by_type = get_class($userData);
...
if (isset($destination) && $destination->id) {
            ...
            $existingFolder = ErpDocumentDriveFolder::whereNull('parent_id')
                ->where('name', $source->name)
                ->where('created_by', $userData->id)->where('created_by_type',get_class($userData))->first();
            $existingFile = ErpDocumentDriveFile::whereNull('folder_id')
            ->where('created_by', $userData->id)->where('created_by_type',get_class($userData))
                ->where('name', $source->name)
                ->first();
    ...
    public function shareMultiple(Request $request)
    {
        ...
        $validatedData = $request->validate(
            [
                ...
                'selected_users.*' => 'exists:users,id', // Ensure the users exist in the database
            ...
            [
                ...
                'selected_users.*.exists' => 'One or more selected users are invalid.',
    ...
    public function share(Request $request)
    {
        ...
        $validatedData = $request->validate(
            [
                ...
                'users.*' => 'exists:users,id', // Ensure the users exist in the database
            ],
            [
