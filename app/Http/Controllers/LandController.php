...
class LandController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // $leases = Lease::where('organization_id', $organization_id)
        //     // Legals created by the user
        //     ->where('user_id', $user_id)
        //     ->where('type', $type)
        //     ->with('land')->orderBy('created_at', 'desc')
        ...
        if ($request->start_date) {
            ...
            $active_leases = LandLease::whereBetween('lease_end_date', [$startDate, $endDate])
                ->whereRaw('DATEDIFF(lease_end_date, ?) > 30', [$today])
                ->count();

            // Expiring leases within the range
            $expiring_leases = LandLease::whereBetween('lease_end_date', [$startDate, $endDate])
                ->whereRaw('DATEDIFF(lease_end_date, ?) < 30', [$today])
                ->count();

            // Expired leases within the range
            $expired_leases = LandLease::whereBetween('lease_end_date', [$startDate, $endDate])
                ->whereRaw('DATEDIFF(?, lease_end_date) > 0', [$today])
                ->count();

            // Leases with customer data within the range
            $leases = LandLease::with('customer')
                ->whereBetween('lease_end_date', [$startDate, $endDate])
                ->whereRaw('DATEDIFF(lease_end_date, ?) > 0', [$today])
                ->get();

            // Lease revenue summary within the range
            $lease_revenue_summary = LandLease::whereBetween('lease_end_date', [$startDate, $endDate])
                ->sum('total_amount');
        } else {
            // Original queries when $request is not present
            $active_leases = LandLease::whereRaw('DATEDIFF(lease_end_date, ?) > 30', [$today])
                ->count();

            $expiring_leases = LandLease::whereRaw('DATEDIFF(lease_end_date, ?) < 30', [$today])
                ->count();

            $expired_leases = LandLease::whereRaw('DATEDIFF(?, lease_end_date) > 0', [$today])
                ->count();
            ...
            $lease_revenue_summary = LandLease::sum('total_amount');
        }

        $recoveries = Recovery::with('lease')->where('organization_id', $organization_id)
            // Legals created by the user
            ->where('user_id', $user_id)
            ->where('type', $type)
            ->when($startDate && $endDate, function ($query) use ($startDate, $endDate) {
            ...
            ->orderBy('created_at', 'desc')->take(5)->get()->map(function ($recovery) {
                $recovery->due_amount = $recovery->lease_cost - $recovery->received_amount;
                $recovery->due_date = Carbon::parse($recovery->created_at)->addYears($recovery->lease_time)->format('d-M-Y');
                return $recovery;
            });

        // Fetch all locations with plots and their associated locations
        $locations = LandParcel::with(['plots.locations', 'locations'])->where('document_status', '!=', 'draft')->get();
    ...
    public function getDashboardRevenueReport()
    {
        ...
        Recovery::where('organization_id', $organization_id)
            // Legals created by the user
            ->where('user_id', $user_id)
            ->where('type', $type)->with('lease')
            ->get() // Fetch data
            ->map(function ($recovery) use (&$totals) {
                // Calculate due amount
                $due_amount = $recovery->lease_cost - $recovery->received_amount;

                // Calculate due date based on lease time
                $due_date = Carbon::parse($recovery->lease->created_at)->addYears($recovery->lease_time);

                // Ensure due_date is valid and overdue calculation is correct
                $now = Carbon::now();
                $is_overdue = $now->gt($due_date);

                // Determine overdue amount (if due date is past and due amount is greater than 0)
                $overdue_amount = $is_overdue && $due_amount > 0 ? $due_amount : 0;

                // Add to totals
                $totals['total_revenue'] += $recovery->received_amount;
                $totals['total_overdue'] += $overdue_amount;
                $totals['total_pending'] += $due_amount;

                // Add these properties to the object for further use
                $recovery->due_amount = $due_amount;
                $recovery->due_date = $due_date->format('d-M-Y');
                $recovery->overdue_amount = $overdue_amount;
                $recovery->revenue_amount = $recovery->received_amount;
