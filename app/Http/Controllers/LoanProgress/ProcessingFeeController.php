...
class ProcessingFeeController extends Controller
{
    ...
    public function viewHomeLoan($id)
    {
        ...
        return view('loanProgress.viewHomeLoan', compact('homeLoan','isPostingRequired','banks', 'ledgers','groups','allledgers','occupation', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'behalf_users', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons', 'logs'));
    }
...
$buttons = Helper::actionButtonDisplayForLoan($vehicleLoan->series, $vehicleLoan->approvalStatus,$vehicleLoan->id,$vehicleLoan->loan_amount,$vehicleLoan->approval_level,$vehicleLoan->loanable_id,$vehicleLoan->loanable_type);        $logs = Helper::getLogs($id);
        ...
        return view('loanProgress.viewVehicleLoan', compact('groups','allledgers','vehicleLoan', 'banks', 'ledgers', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons', 'logs'));
    }
    ...
    public function viewTermLoan($id)
    {
        ...
        $ledgers = Ledger::withDefaultGroupCompanyOrg()
            ...
            return view('loanProgress.viewTermLoan', compact('groups','allledgers','banks', 'ledgers', 'termLoan', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons', 'logs'));
    }
    ...
    {
        try {
            $data = FinancialPostingHelper::loanVoucherPosting($request -> book_id ?? 0, $request -> document_id ?? 0, "get",$request->remakrs);
            return response() -> json([
                'status' => 'success',
                'data' => $data
            ]);
    ...
    public function postInvoice(Request $request)
    {
        try {
            DB::beginTransaction();
            $data = FinancialPostingHelper::loanVoucherPosting($request -> book_id ?? 0, $request -> document_id ?? 0, "post",$request->remakrs);
            if ($data['status']) {
                DB::commit();
            } else {
                DB::rollBack();
