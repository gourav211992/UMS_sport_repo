...
use App\Models\LoanSanctionLetter;
...
class SanctionLetterController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $loans->where('loan.approvalStatus', 'Approved')
            ->orWhere('loan.approvalStatus', 'approval_not_required')
            ->orWhere('loan.approvalStatus', 'Assessed')
            ->orWhere('loan.approvalStatus', 'assessment');
            ...
            return DataTables::of($loans)
                ...
                ->addColumn('action', function ($loan) {
                    $view_route = '';
                    if ($loan->loan->type == 1) {
                        $view_route = 'loanSanctionLetter.viewHomeLoan';
                    } elseif ($loan->loan->type == 2) {
                        $view_route = 'loanSanctionLetter.viewVehicleLoan';
                    } else {
                        $view_route = 'loanSanctionLetter.viewTermLoan';
                    }
        ...
        return view('loanProgress.sanctionLetter.list', compact('loans'));
    }
    ...
    public function viewVehicleLoan($id)
    {
        ...
        return view('loanProgress.viewVehicleLoan', compact('banks', 'ledgers','vehicleLoan', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons', 'logs'));
    ...
    public function viewTermLoan($id)
    {
        ...
        $buttons = Helper::actionButtonDisplayForLoan($termLoan->series, $termLoan->approvalStatus);
        $logs = Helper::getLogs($id);
        $banks = Bank::withDefaultGroupCompanyOrg()->with('bankDetails')
            ->get();
        $groupId = Group::where('name', 'Stock-in-Hand')->value('id');
        ...
        return view('loanProgress.viewTermLoan', compact('banks', 'ledgers','termLoan', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons', 'logs'));

    }

    function loanAccept(Request $request)
    {
        ...
        if ($request->hasFile('document')) {
            ...
            $directoryPath = 'loan_documents/sanctioned';
            ...
            HomeLoan::find($request->loan_application_id)->update(['approvalStatus' => 'sanctioned']);

            LoanSanctionLetter::create([
        ...
        // dd($documentPath);
        return redirect()->route('loanProcessingFee.index');
    ...
    function loanReturn(Request $request)
    {
        ...
        if ($request->hasFile('document')) {
            ...
            LoanReturn::create([
                'loan_application_id' => $request->loan_application_id,
                'status' => 'draft',
                'return_page_status' => 'sanction_letter',
                'doc' => $documentPath,
                'remarks' => $request->remarks
            ]);
            $docs[] = $documentPath;
        ...
        return redirect()->route('loanSanctionLetter.view' . ucwords($request->loan_type) . 'Loan', [$request->loan_application_id]);
    ...
    {
        return view('loanProgress.sanctionLetter.view');
    }
}
