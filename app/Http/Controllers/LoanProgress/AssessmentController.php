...
class AssessmentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $loans = ErpLoanAppraisal::with('loan')
            ...
            $loans->where('loan.approvalStatus', 'appraisal');
            ...
            return DataTables::of($loans)
                ...
                ->addColumn('action', function ($loan) {
                    $view_route = '';
                    if ($loan->loan->type == 1) {
                        $view_route = 'loanAssessment.viewHomeLoan';
                    } elseif ($loan->loan->type == 2) {
                        $view_route = 'loanAssessment.viewVehicleLoan';
                    } else {
                        $view_route = 'loanAssessment.viewTermLoan';
                    }
        ...
        return view('loanProgress.assessment.list', compact('loans'));
    }
    ...
    public function viewHomeLoan($id)
    {
       ...
       $book_type = Helper::getBookSeriesNew($firstService -> alias, $parentURL)->get();
        ...
        return view('loanProgress.viewHomeLoan', compact('banks', 'ledgers','homeLoan', 'occupation', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons','logs'));
    }
...
$buttons = Helper::actionButtonDisplayForLoan($vehicleLoan->series, $vehicleLoan->approvalStatus,$vehicleLoan->id,$vehicleLoan->loan_amount,$vehicleLoan->approval_level,$vehicleLoan->loanable_id,$vehicleLoan->loanable_type);        $logs = Helper::getLogs($id);
        ...
        return view('loanProgress.viewVehicleLoan', compact(
            'banks', 'ledgers',
            'vehicleLoan',
            'series',
            'book_type',
    ...
    public function viewTermLoan($id)
    {
        ...
        return view('loanProgress.viewTermLoan', compact('banks', 'ledgers','termLoan','overview', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons','logs'));
    }

    function assessmentProceed(Request $request)
    {
        $request->validate([
        ...
        if ($request->hasFile('document')) {
            ...
            $directoryPath = 'loan_documents/assessment';
            ...
            $status = ConstantHelper::ASSESSMENT ? Helper::checkApprovalLoanRequired($data->book_id) : ConstantHelper::ASSESSMENT;

            if($status==ConstantHelper::ASSESSMENT || $status == ConstantHelper::ASSESSED){
                if ($data->approvelworkflow->count() > 0) { // Check if the relationship has records
                    foreach ($data->approvelworkflow as $approver) {
                        if ($approver->user) { // Check if the related user exists
                            ...
                            LoanNotificationController::notifyLoanSubmission($approver_user, $data);
                        }
                    }
                }
            }

            HomeLoan::find($request->loan_application_id)->update(['approvalStatus' => $status]);

            LoanAssessment::create([
    ...
    function loanReturn(Request $request)
    {
        ...
        if ($request->hasFile('document')) {
            // Define the storage path
            $directoryPath = 'assessment/return';
            ...
            // Store the file in the 'assessment/assessment' folder
            $documentPath = $request->file('document')->store($directoryPath, 'public');
        ...
        return redirect()->route('loanAssessment.view' . ucwords($request->loan_type) . 'Loan', [$request->loan_application_id]);
    }
    ...
    function loanReject(Request $request)
    {
        ...
        if ($request->hasFile('document')) {
            // Define the storage path
            $directoryPath = 'assessment/reject';
    ...
    {
        return view(view: 'loanProgress.assessment.view');
    }

    function viewLoanAssessment()
    {
        // return view()
    }
}
