...
class PoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            ->addColumn('vendor_name', function ($row) {
                return $row->vendor?->display_name ?? 'NA';
            })
    ...
    public function addItemRow(Request $request)
    {
        ...
        $html = view('procurement.po.partials.item-row',compact('rowCount'))->render();
            ...
            if (isset($request->all()['components']) && count($request->all()['components'])) {
                ...
                foreach($poItemArr as $_key => $poItem) {
                    ...
                    $piItems = PiItem::whereIn('id',$pi_item_ids)
                                ...
                                ->where(function($piItemQuery) use($poDetail) {
                                    if(count($poDetail->attributes)) {
                                        $piItemQuery->whereHas('attributes', function($piAttributeQuery) use($poDetail) {
                                            foreach($poDetail->attributes as $piAttribute) {
                                                $piAttributeQuery->where('item_attribute_id',$piAttribute->item_attribute_id)
    ...
    public function getItemDetail(Request $request)
    {
        ...
        $html = view('procurement.po.partials.comp-item-detail',compact('item','selectedAttr','remark','uomName','qty','delivery','specifications','piItems','totalPoQnt'))->render();
        return response()->json(['data' => ['html' => $html], 'status' => 200, 'message' => 'fetched.']);
    }
    ...
    public function getPi(Request $request)
    {
        ...
        if($this->type == 'supplier-invoice') {
            $piItems = PoItem::where(function($query) use ($seriesId,$applicableBookIds,$docNumber,$itemId,$vendorId, $departmentId) {
                        $query->whereHas('item');
                        $query->whereHas('po', function($pi) use ($seriesId,$applicableBookIds,$docNumber,$vendorId, $departmentId) {
                            $pi->where('type','po');
        ...
        $piItems = PiItem::where(function($query) use ($seriesId,$applicableBookIds,$docNumber,$itemId,$vendorId, $departmentId) {
                        $query->whereHas('item');
                        $query->whereHas('pi', function($pi) use ($seriesId,$applicableBookIds,$docNumber,$departmentId) {
                            $pi->withDefaultGroupCompanyOrg();
                            $pi->whereIn('document_status', [ConstantHelper::APPROVED, ConstantHelper::APPROVAL_NOT_REQUIRED]);
                            if($seriesId) {
                                $pi->where('book_id',$seriesId);
        ...
        $html = view('procurement.po.partials.pi-item-list', ['piItems' => $piItems])->render();
    ...
    public function processPiItem(Request $request)
    {
        ...
        if($this->type == 'supplier-invoice') {
            ...
            // $piItemGrouped = DB::table(function ($query) use ($ids) {
            //     $query->from('erp_pi_items')
            //         ->join('erp_pi_item_attributes', 'erp_pi_items.id', '=', 'erp_pi_item_attributes.pi_item_id')
            //         ->select(
            //             'erp_pi_items.id as pi_item_id',
            //             'erp_pi_items.item_id',
            //             'erp_pi_items.uom_id',
            //             DB::raw("GROUP_CONCAT(CONCAT(erp_pi_item_attributes.item_attribute_id, ':', erp_pi_item_attributes.attribute_value) ORDER BY erp_pi_item_attributes.item_attribute_id SEPARATOR ', ') as attributes"),
            //             'erp_pi_items.indent_qty',
            //             'erp_pi_items.order_qty'
            //         )
            //         ->whereIn('erp_pi_items.id',$ids)
            ...
            // ->groupBy('item_id', 'uom_id', 'attributes')
            // ->get();
            $html = view('procurement.po.partials.item-row-pi', ['piItemGrouped' => $piItemGrouped])->render();
        }
