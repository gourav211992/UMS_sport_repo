...
class ExamFeeAllController extends AdminController
{
    ...
    //         ->where('stream_id',$scrutiny->stream_id)
    //         ->get();
                
    //     return view('ums.studentform.view',['scrutiny'=>$scrutiny,'sublist'=>$sublist,'id'=>$slug]);
    // }
    ...
    public function view_challenge_form(Request $request, $slug)
    {
        // Fetch the scrutiny record
        $scrutiny = Scrutiny::where('id', $slug)->orderBy('id', 'DESC')->first();
    
        // Check if scrutiny is null before proceeding
        if (!$scrutiny) {
            return back()->with('error', 'Scrutiny record not found.');
        }
    
        // Ensure that sub_code exists and is not empty
        if (empty($scrutiny->sub_code)) {
            return back()->with('error', 'No subject code found for this scrutiny.');
        }
        ...
        return view('ums.studentform.view', [
            'scrutiny' => $scrutiny,
            'sublist' => $sublist,
            'id' => $slug
        ]);
