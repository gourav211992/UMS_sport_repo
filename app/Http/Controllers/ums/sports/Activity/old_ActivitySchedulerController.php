...
use App\Models\ums\SportRegister;
...
{
    $section = Section::where('batch_id', $request->batch_name)->get();
...
public function get_section_group(Request $request)
{
    ...
    if (is_numeric($sectionValue)) {
        $group = MasterGroup::where('section_id', $sectionValue)->get();
    } else {
        $group = MasterGroup::where('section_name', $sectionValue)->get();
    }
