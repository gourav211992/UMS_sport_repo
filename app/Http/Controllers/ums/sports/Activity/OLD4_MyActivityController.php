...
class MyActivityController extends Controller
{
    ...
    //     $allBatches = batch::select('batch_name')->get()->unique('batch_name')->values();
    ...
    //     if ($request->filled('activity_name') && $request->activity !== 'Select') {
    ...
    //     if ($request->filled('batch_name') && $request->batch !== 'Select') {
    //         $finalActivities = $finalActivities->filter(function ($item) use ($request) {
    ...
    public function index(Request $request)
{
    ...
    if ($request->filled('batch') && $request->batch !== 'Select') {
        $finalActivities = $finalActivities->filter(fn($item) => $item->batch === $request->batch);
    }

    if ($request->filled('section') && $request->section !== 'Select') {
        $finalActivities = $finalActivities->filter(fn($item) => $item->section === $request->section);
    }

    if ($request->filled('group') && $request->group !== 'Select') {
        $finalActivities = $finalActivities->filter(fn($item) => $item->group === $request->group);
    }
