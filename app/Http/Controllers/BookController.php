        ...
        <button type="button"
            ...
            <div class="dropdown-menu dropdown-menu-end">
                <a class="dropdown-item" href="'. route('bookEdit', $row -> id) .'">
                    <i data-feather="edit-3" class="me-50" style="color: black;"></i>
...
//        dd($userType);
        $companyIds = Organization::whereIn('id', [$authUser?->organization_id])->where('status', ConstantHelper::ACTIVE)->get()->pluck('company_id');
//        $companies = Organization::where('status', ConstantHelper::ACTIVE)->get();
//        dd($companyIds);
        $companies = OrganizationCompany::whereIn('id', [$companyIds])->with('organizations')->get();
//        dd($companies[0]->organizations);
        $people = Admin::where('organization_id',$authUser?->organization_id)->get();
//        $people = [];
        return view('book.create-book', compact('companies', 'people', 'services','orgIds'));
    }
...
//        dd($request->all());
        ...
        $incomingData = [
            'level_organization_id' => $request->input('level_organization_id'),
            'user' => $request->input('user'),
        ];

        // Log incoming request data
        Log::info('Incoming request data: ', $incomingData);

        // Define the expected structure
        $expectedStructure = [
            'level_organization_id' => 'array of numeric integers',
            'user' => 'array of arrays with numeric integers',
        ];

        // Log expected structure
        Log::info('Expected structure: ', $expectedStructure);
...
//            }
        ...
        if (isset($book)) {
            ...
            foreach ($book -> levels as $approvalLevel => &$approver) {
                ...
                $modelName = isset(ConstantHelper::SERVICE_ALIAS_MODELS[$serviceAlias]) ? ConstantHelper::SERVICE_ALIAS_MODELS[$serviceAlias] : '';
                if (isset($modelName)) {
                    $model = resolve('App\\Models\\' . 'ums\\Admin');
