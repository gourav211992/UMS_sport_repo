...
class HomeLoanController extends Controller
{
    ...
    public function create(StoreHomeLoanRequest $request)
    {
        try {
            ...
            if (!$homeLoan) {
                return redirect("loan/index")->with('error', 'Something went wrong.');
            }
            if ($homeLoan) {
            ...
            DB::commit();
            return redirect("loan/index")->with('success', 'Home Loan created/updated successfully!');

        } catch (\Exception $e) {
            DB::rollBack();
            return redirect("loan/index")->with('error', 'Something went wrong: ' . $e->getMessage());
        }
    }
...
$buttons = Helper::actionButtonDisplayForLoan($homeLoan->series, $homeLoan->approvalStatus,$homeLoan->id,$homeLoan->loan_amount,$homeLoan->approval_level,$homeLoan->loanable_id,$homeLoan->loanable_type);        
    ...
    public function destroy($id)
    {
        ...
        return redirect("loan/index")->with('success', 'Home loan and related records deleted successfully.');
    }
}
