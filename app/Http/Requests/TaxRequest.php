...
class TaxRequest extends FormRequest
{
    ...
    {
        return [
            'tax_group' => 'required|string|max:100',
            'description' => 'nullable|string',
            'status' => 'required|string',
            'applicability_type' => 'required|string',
            'group_id' => 'nullable|exists:groups,id',
            'company_id' => 'nullable',
            'organization_id' => 'nullable|exists:organizations,id',
            'tax_details' => 'required|array',
            'tax_details.*.id' => 'nullable|integer|exists:erp_tax_details,id',
            'tax_details.*.tax_type' => 'nullable|in:SGST,CGST,IGST,TDS,TCS,VAT',
            'tax_details.*.tax_percentage' => 'required|numeric|min:0|max:100',
            'tax_details.*.place_of_supply' => 'nullable|in:Intrastate,Interstate,Overseas',
