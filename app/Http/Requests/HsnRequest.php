...
class HsnRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            'tax_patterns' => function ($attribute, $value, $fail) {
                $groups = [];
                    ...
                    if (isset($pattern['from_price']) && isset($pattern['upto_price']) && $taxGroupId) {
                        ...
                        foreach ($groups[$taxGroupId] as $range) {
                            if ($from <= $range['upto'] && $upto >= $range['from']) {
                                return $fail('The price ranges overlap within the same tax group. Ensure that no two price ranges overlap within the same tax group.');
                            }
                        }
    ...
    public function messages()
    {
        return [
            ...
            'tax_patterns' => 'The price ranges overlap within the same tax group. Ensure that no two price ranges overlap within the same tax group.',
        ];
    }
}
