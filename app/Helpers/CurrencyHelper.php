...
class CurrencyHelper
{
    ...
    public static function getCurrencyExchangeRates(int $currencyId, string $transactionDate) : array
    {
        ...
        if (isset($group)) {
            ...
            if (isset($groupCurrency)) {
                if ($currencyId === $groupCurrency -> id) {
                    $groupExchangeRate = 1;
                } else {
                    $groupExchangeRate = self::getCurrencyExchangeRate($currencyId, $groupCurrency -> id, $transactionDate, $groupId, $companyId, $organizationId);
                }
                //Group Exchange Rate Check
                if (!isset($groupExchangeRate)) {
                    return array(
                        'status' => false,
                        'message' => 'Exchange Rate not mapped from ' .$partyCurrency -> short_name .' to ' . $groupCurrency -> short_name,
                        'data' => null
                    );
                }
            } else {
        ...
        if (isset($company)) {
            ...
            if (isset($companyCurrency)) {
                if ($currencyId === $companyCurrency -> id) {
                    $companyExchangeRate = 1;
                } else {
                    $companyExchangeRate = self::getCurrencyExchangeRate($currencyId, $companyCurrency -> id, $transactionDate, $groupId, $companyId, $organizationId);
                }
                //Company Exchange Rate Check
                if (!isset($companyExchangeRate)) {
                    return array(
                        'status' => false,
                        'message' => 'Exchange Rate not mapped from ' . $partyCurrency ?-> short_name .' to '. $companyCurrency -> short_name,
                        'data' => null
                    );
                }
            } else {
