            ...
            if ($data->series_numbering === ConstantHelper::DOC_NO_TYPE_AUTO) {
                ...
                if ($data->reset_pattern === ConstantHelper::DOC_RESET_PATTERN_NEVER) {
                    ...
                    $currentDocNo = $model->withDefaultGroupCompanyOrg()->where('book_id', $book_id)
                        ->where('doc_no','!=',null)->orderBy('doc_no', 'DESC')->pluck('doc_no')->first() ?? $startFrom;
//                    dd($model->withDefaultGroupCompanyOrg()->get());
...
//                dd($currentDocNo);
    ...
    public static function getAuthenticatedUser()
    {
        ...
        if (Auth::guard('sports')->check()) {
            return Auth::guard('sports')->user();
...
//        dd($authUser);
        if ($authUser) {
            $data = ['user_id' => Auth::guard('admin')->user()->id, 'user_type' => 'App\Models\ums\Admin', 'type' => Auth::guard('admin')->user()->authenticable_type];
            return $data;
        } else {
            $data = ['user_id' => Auth::guard('web2')->user()->id, 'user_type' => 'App\Models\ums\Admin'];
            return $data;
        }
    }
    ...
    {
        $adminIds = \App\Models\ums\Admin::where('organization_id', $organizationId)
            ->get()
            ->pluck('id');

        $admin = self::getAuthenticatedUser();

        $admins = \App\Models\ums\Admin::where('organization_id', $organizationId)
            ->whereIn('id', $adminIds)
            ->get();
    ...
    {
        $authUser = Helper::getAuthenticatedUser(); // Assuming this returns the authenticated admin user.
        // $authUser will now refer to the authenticated admin
        ...
        if (!isset($organizationMenu)) {
            ...
            // Check for authenticated admin instead of 'userType'
            if ($authUser || $authUser->role === 'admin') {
                if ($menuAlias) {
                    $organizationServices = OrganizationService::withDefaultGroupCompanyOrg()
                        ->where('alias', $menuAlias)->get();
                    return [
                    ...
                    $services = EmployeeBookMapping::where('service_menu_id', $organizationMenu?->serviceMenu->id)
                        ->where('employee_id', $authUser->id)->first();

                    if (!isset($services)) { // Assign all services and books if no record is found
                        $serviceIds = $organizationMenu?->serviceMenu?->erp_service_id ?? [];
                        $organizationServices = OrganizationService::withDefaultGroupCompanyOrg()
                            ->whereIn('service_id', $serviceIds)
                            ->when($selectedServiceAlias, function ($aliasQuery) use ($selectedServiceAlias) {
                        ...
                        if ($organizationServices && count($organizationServices) > 0) {
                            $organizationServices = $organizationServices->filter(function ($orgService) use ($bookIds) {
                                $isBookExist = Book::withDefaultGroupCompanyOrg()
                                    ->whereIn('id', $bookIds)
    ...
    public static function setMenuAccessToEmployee(String $menuName, String $menuAlias, array $serviceIds): string
    {
        ...
        try {
            ...
            foreach ($employees as $employee) {
                $employee = Admin::find($employee->id);
